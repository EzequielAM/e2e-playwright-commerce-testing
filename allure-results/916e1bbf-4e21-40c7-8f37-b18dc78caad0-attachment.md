...
```ts
  ...
  6  | 
  7  |         // 1. Interceptamos la petición de la imagen de la mochila original
  8  |         // Swag Labs descarga sus imágenes desde su estructura estática. Capturamos ese recurso.
  9  |         await page.route('**/sauce-backpack-1200x1500.jpg', async (route) => {
  10 | 
  11 |             // Reemplazamos la imagen real por un placeholder externo mediante el fulfillment de la ruta
  12 |             await route.fulfill({
  13 |                 status: 200,
  14 |                 contentType: 'image/jpeg',
  15 |                 url: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=150' // Una foto cualquiera de internet
  16 |             });
  17 |         });
  18 | 
  19 |         // 2. Ejecutamos el flujo común de login que ya tenés dominado
  ...
  24 | 
  25 |         // 3. Validamos que el login fue exitoso y estamos en el inventario
  26 |         await expect(page).toHaveURL('https://www.saucedemo.com/inventory.html');
  27 | 
  28 |         // 4. Verificación de UI: La imagen sigue existiendo en el DOM,
  29 |         // pero Playwright inyectó nuestro mock en el tráfico de red.
  30 |         const firstProductImage = page.locator('.inventory_item_img img').first();
  31 |         await expect(firstProductImage).toBeVisible();
  32 |     });
  33 | 
  34 |     test('Simular caída del servidor (Error 500) al cargar componentes de la app', async ({ page }) => {
  35 | 
  36 |         // Interceptamos cualquier intento de cargar el script principal de la aplicación para simular caída
  37 |         await page.route('**/main.js', async (route) => {
  ...
  46 | 
  47 |         // Al fallar componentes críticos, podemos verificar que la página sufra un comportamiento controlado
  48 |         // o validar los botones de login. En este caso probamos que el formulario base siga en pantalla.
  49 |         await expect(page.locator('#login-button')).toBeVisible();
  50 |     });
  51 | });
  52 | 
     |                                                                         ^ Error: locator.textContent: Test timeout of 30000ms exceeded.
